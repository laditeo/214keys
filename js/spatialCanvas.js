...
let collapsed = false;
let landscape = false;
const SPATIAL_CANVAS_EXPAND_MIN_WIDTH = 1100;

function isSpatialCanvasExpandedViewport() {
  return window.matchMedia("(orientation: landscape)").matches && window.innerWidth >= SPATIAL_CANVAS_EXPAND_MIN_WIDTH;
}
...
/** @type {{ startDistance: number, midpoint: { x: number, y: number }, startZoom: number, worldAnchor: { x: number, y: number } } | null} */
let pinchStart = null;
/** @type {number[] | null} */
let pinchPointerIds = null;
/** @type {{ id: number, points: { x: number, y: number }[] } | null} */
let lassoPointer = null;
/** @type {{ id: number, clientX: number, clientY: number } | null} */
...
function bindSpawnButton(spawnBtn, spawnMode) {
  // Лонг-тап на Android иначе вызывает контекстное меню (эмуляция правого клика).
  spawnBtn.addEventListener("contextmenu", (event) => event.preventDefault());
...
function syncTouchPointersFromTouches(touches) {
  if (!stageEl) return;
  touchPointers.clear();
  const rect = stageEl.getBoundingClientRect();
...
function beginLassoAt(clientX, clientY, pointerId) {
  const pt = stagePoint(clientX, clientY);
  lassoPointer = { id: pointerId, points: [pt] };
  lassoEl?.classList.add("is-active");
  setStageCursor("lasso");
}

function maybeStartTouchLasso(touch) {
  if (!touchLassoPending || touchLassoPending.id !== touch.identifier || lassoPointer) return false;
  const dist = Math.hypot(touch.clientX - touchLassoPending.clientX, touch.clientY - touchLassoPending.clientY);
  if (dist <= 6) return false;
  beginLassoAt(touchLassoPending.clientX, touchLassoPending.clientY, touch.identifier);
...
function beginLasso(event) {
  beginLassoAt(event.clientX, event.clientY, event.pointerId);
}
...
function appendLassoPoint(clientX, clientY) {
  ...
  const minDist = prefersTouchGestures() ? 2 : 3;
...
function finishLasso() {
  ...
  if (pts.length > (prefersTouchGestures() ? 2 : 4)) {
    ...
    if (span > (prefersTouchGestures() ? 6 : 8)) {
      selectedIds.clear();
...
const SPAWN_SPIRAL_ICON =
  "<svg class=\"spatial-canvas__spawn-icon\" viewBox=\"0 0 24 24\" fill=\"none\" aria-hidden=\"true\"><path d=\"M12.00 12.00 L12.15 12.04 L12.26 12.16 L12.31 12.34 L12.27 12.55 L12.14 12.75 L11.92 12.91 L11.62 13.00 L11.27 12.98 L10.91 12.83 L10.58 12.56 L10.33 12.16 L10.20 11.67 L10.22 11.12 L10.42 10.57 L10.79 10.06 L11.32 9.66 L11.99 9.41 L12.74 9.35 L13.51 9.53 L14.24 9.93 L14.86 10.55 L15.29 11.36 L15.50 12.30 L15.42 13.30 L15.06 14.27 L14.42 15.15 L13.52 15.83 L12.43 16.25 L11.23 16.36 L10.00 16.12 L8.85 15.53 L7.88 14.61 L7.17 13.43 L6.81 12.06 L6.85 10.60 L7.30 9.16 L8.15 7.87 L9.35 6.84 L10.84 6.17 L12.49 5.92 L14.18 6.14 L15.79 6.84 L17.18 7.98 L18.22 9.49 L18.83 11.27 L18.92 13.19 L18.46 15.10 L17.48 16.85 L16.03 18.29 L14.20 19.30 L12.12 19.78 L9.95 19.66 L7.86 18.94 L6.01 17.65 L4.56 15.87 L3.63 13.72 L3.33 11.35 L3.69 8.95 L4.72 6.71\" stroke=\"currentColor\" stroke-width=\"1.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/></svg>";

const SPAWN_SUNFLOWER_ICON =
  "<svg class=\"spatial-canvas__spawn-icon\" viewBox=\"0 0 24 24\" fill=\"none\" aria-hidden=\"true\"><g fill=\"currentColor\"><circle cx=\"13.55\" cy=\"12.00\" r=\"1.80\"/><circle cx=\"10.02\" cy=\"13.81\" r=\"1.75\"/><circle cx=\"12.30\" cy=\"8.55\" r=\"1.70\"/><circle cx=\"14.50\" cy=\"15.25\" r=\"1.65\"/><circle cx=\"7.42\" cy=\"11.19\" r=\"1.60\"/><circle cx=\"16.34\" cy=\"9.24\" r=\"1.55\"/><circle cx=\"10.55\" cy=\"17.40\" r=\"1.50\"/><circle cx=\"9.23\" cy=\"6.67\" r=\"1.45\"/><circle cx=\"18.00\" cy=\"14.19\" r=\"1.40\"/><circle cx=\"5.75\" cy=\"14.58\" r=\"1.35\"/><circle cx=\"15.01\" cy=\"5.57\" r=\"1.30\"/><circle cx=\"14.22\" cy=\"19.09\" r=\"1.25\"/><circle cx=\"5.29\" cy=\"8.11\" r=\"1.20\"/><circle cx=\"19.87\" cy=\"10.27\" r=\"1.15\"/><circle cx=\"7.20\" cy=\"18.83\" r=\"1.10\"/><circle cx=\"10.89\" cy=\"3.44\" r=\"1.05\"/><circle cx=\"18.81\" cy=\"17.74\" r=\"1.00\"/><circle cx=\"2.84\" cy=\"12.38\" r=\"0.95\"/></g></svg>";
...
function renderSpawnDock() {
  if (!spawnDockEl) return;
  stopSpawnStreak();
  spawnDockEl.innerHTML = "";
  spawnDockEl.append(
    createSpawnButton({
      glyph: "+",
...
const PANEL_WIDTH_KEY = "214keys-spatial-panel-width";
const PANEL_WIDTH_MIN = 220;

function panelWidthMax() {
  return Math.max(PANEL_WIDTH_MIN, Math.round(window.innerWidth * 0.85));
}

function clampPanelWidth(px) {
  return Math.min(panelWidthMax(), Math.max(PANEL_WIDTH_MIN, Math.round(px)));
}
...
function applyPanelWidth() {
  ...
  let stored = 0;
  try {
    stored = parseFloat(localStorage.getItem(PANEL_WIDTH_KEY) || "");
  } catch {
    stored = 0;
  }
  if (!Number.isFinite(stored) || stored <= 0) return;
  rootEl.style.setProperty("--canvas-panel-width", `${clampPanelWidth(stored)}px`);
}
...
function bindPanelResize() {
  ...
  const onMove = (event) => {
    ...
    // Панель прижата к правому краю: тянем ручку влево → шире.
    const next = clampPanelWidth(startWidth + (startClientX - event.clientX));
    rootEl.style.setProperty("--canvas-panel-width", `${next}px`);
  };

  const onUp = (event) => {
    if (event.pointerId !== resizePointerId) return;
    resizePointerId = null;
    resizeHandleEl.classList.remove("is-dragging");
    ...
    try {
      const w = Math.round(panelEl.getBoundingClientRect().width);
  ...
  resizeHandleEl.addEventListener("pointerdown", (event) => {
    ...
    resizeHandleEl.classList.add("is-dragging");
    try {
...
function beginPinch() {
  ...
  pinchStart = {
    ...
    startZoom: zoom,
    worldAnchor: {
      x: (mid.x - state.viewport.x) / zoom,
      y: (mid.y - state.viewport.y) / zoom,
    },
  };
  panPointer = null;
  rmbZoomPointer = null;
...
function applyPinch() {
  ...
  const nextZoom = Math.min(
    MAX_ZOOM,
    Math.max(MIN_ZOOM, pinchStart.startZoom * ratio),
  );
  state.viewport.zoom = nextZoom;
  state.viewport.x = midpoint.x - pinchStart.worldAnchor.x * nextZoom;
...
function isUiChromeTarget(target) {
  return Boolean(
    target?.closest?.(
      ".spatial-fence__label, .spatial-fence__resize, .spatial-popup, .spatial-popup *, .modal-canvas__tools, .modal-canvas__tools *, .modal-canvas__resize, .spatial-confirm, .spatial-confirm *, .spatial-canvas__spawn, .spatial-canvas__spawn-group",
    ),
  );
}
...
function isInteractiveTarget(target) {
  return Boolean(
    target?.closest?.(
      ".spatial-card, .spatial-fence__label, .spatial-fence__resize, .spatial-popup, .spatial-popup *, .modal-canvas__tools, .modal-canvas__tools *, .modal-canvas__resize, .spatial-confirm, .spatial-confirm *, .spatial-canvas__spawn, .spatial-canvas__spawn-group",
    ),
  );
}
...
function onStagePointerDown(event) {
  ...
  if (prefersTouchGestures() && event.pointerType === "touch") return;
...
function syncOrientation() {
  const nextLandscape = isSpatialCanvasExpandedViewport();
...
export function initSpatialCanvas(options) {
  ...
  spawnDockEl = document.getElementById("modal-canvas-spawn");
  panelEl = document.getElementById("modal-canvas-panel");
  resizeHandleEl = document.getElementById("modal-canvas-resize");
  ...
  toolbarEl = document.getElementById("modal-canvas-tools-tl");
...
export function showSpatialCanvas() {
  if (!rootEl) return;
  rootEl.hidden = false;
  landscape = isSpatialCanvasExpandedViewport();
