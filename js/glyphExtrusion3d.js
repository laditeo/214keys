...
const GEOMETRY_CACHE_TAG = "extrude-v8-outline";

/**
 * Порог auto-smooth: рёбра, где нормали смежных граней расходятся сильнее этого
 * угла, считаются острыми и остаются гранёными; более пологие (тупые) — сглаживаются.
 */
const GLYPH_CREASE_ANGLE = 40 * (Math.PI / 180);
...
/** Толщина 2D-обводки в пикселях экрана (post-process, см. renderSceneWithOutline). */
const DEPTH_OUTLINE_PX = 2.5;
...
const _drawBufferSize = new THREE.Vector2();

function rendererResolution(activeRenderer) {
  if (!activeRenderer) return null;
  activeRenderer.getDrawingBufferSize(_drawBufferSize);
  return _drawBufferSize;
}

/* ------------------------------------------------------------------ *
 * Post-process обводка (2D outline).
 * Меш растеризуется в offscreen-текстуру (с MSAA), затем в экранном
 * пространстве делаем dilation силуэта по альфе и композим обратно.
 * Так обводка ровная и стабильная при любом повороте — в отличие от
 * покадрового раздувания меша (inverted hull).
 * ------------------------------------------------------------------ */
const OUTLINE_POST_VERTEX = /* glsl */ `
  varying vec2 vUv;
  void main() {
    vUv = uv;
...
const OUTLINE_POST_FRAGMENT = /* glsl */ `
  precision highp float;
  uniform sampler2D tScene;
  uniform vec2 texel;          // 1 / размер буфера в пикселях
  uniform float outlineWidth;  // толщина обводки в пикселях буфера
  ...
  void main() {
    ...
    // Ищем максимальную альфу в диске радиусом outlineWidth — так силуэт
    // «растекается» наружу равномерной каймой (несколько колец × секторов).
    float best = 0.0;
    const int RINGS = 4;
    const int SECT = 16;
    for (int r = 1; r <= RINGS; r++) {
      float rad = outlineWidth * (float(r) / float(RINGS));
      for (int s = 0; s < SECT; s++) {
        float a = (float(s) / float(SECT)) * 6.2831853;
        vec2 off = vec2(cos(a), sin(a)) * rad * texel;
        best = max(best, texture2D(tScene, vUv + off).a);
      }
    }

    float outlineA = smoothstep(0.0, threshold, best);
    // Композит: исходный меш ПОВЕРХ каймы (обводка сзади).
    float a = src.a + outlineA * (1.0 - src.a);
    vec3 premul = src.rgb * src.a + outlineColor * outlineA * (1.0 - src.a);
    gl_FragColor = vec4(premul, a);
  }
`;

const outlineRTs = new WeakMap();
let outlineQuadScene = null;
let outlineQuadCamera = null;
let outlineQuadMaterial = null;

function getOutlineRT(activeRenderer, w, h) {
  let rt = outlineRTs.get(activeRenderer);
  if (!rt) {
    ...
    outlineRTs.set(activeRenderer, rt);
...
function ensureOutlineQuad() {
  if (outlineQuadMaterial) return;
  outlineQuadCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
  outlineQuadMaterial = new THREE.ShaderMaterial({
    uniforms: {
      tScene: { value: null },
    ...
    vertexShader: OUTLINE_POST_VERTEX,
    fragmentShader: OUTLINE_POST_FRAGMENT,
    transparent: true,
    blending: THREE.NoBlending,
    depthTest: false,
    depthWrite: false,
  });
  outlineQuadScene = new THREE.Scene();
  outlineQuadScene.add(new THREE.Mesh(new THREE.PlaneGeometry(2, 2), outlineQuadMaterial));
}

/**
 * Рендерит сцену с 2D-обводкой глифов (post-process).
 * @returns {boolean} true, если рендер выполнен здесь.
 */
export function renderSceneWithOutline(activeRenderer, activeScene, activeCamera) {
  if (!activeRenderer || !activeScene || !activeCamera) return false;
  const res = rendererResolution(activeRenderer);
  const w = Math.max(1, Math.round(res.x));
  const h = Math.max(1, Math.round(res.y));

  const rt = getOutlineRT(activeRenderer, w, h);
  const prevTarget = activeRenderer.getRenderTarget();
  const prevAutoClear = activeRenderer.autoClear;

  activeRenderer.setRenderTarget(rt);
  activeRenderer.autoClear = true;
  activeRenderer.setClearColor(0x000000, 0);
  activeRenderer.clear();
  activeRenderer.render(activeScene, activeCamera);
  activeRenderer.setRenderTarget(prevTarget);

  ensureOutlineQuad();
  const px = activeRenderer.getPixelRatio ? activeRenderer.getPixelRatio() : 1;
  outlineQuadMaterial.uniforms.tScene.value = rt.texture;
  outlineQuadMaterial.uniforms.texel.value.set(1 / w, 1 / h);
  outlineQuadMaterial.uniforms.outlineWidth.value = DEPTH_OUTLINE_PX * px;

  activeRenderer.autoClear = true;
  activeRenderer.setClearColor(0x000000, 0);
  activeRenderer.render(outlineQuadScene, outlineQuadCamera);
  activeRenderer.autoClear = prevAutoClear;
  return true;
}
...
function createGlyphMeshForMode(geometry, mode, color) {
  if (mode !== "depth") {
    return new THREE.Mesh(geometry, createGlyphMaterialForMode(mode, color));
  }
  ...
  // 2D-обводка теперь делается post-process (см. renderSceneWithOutline),
  // а не покадровым раздуванием меша — поэтому отдельный outline-меш не нужен.
  group.add(main);
  return group;
}

function createGlyphMeshFromGeometry(geometry, color) {
  return createGlyphMeshForMode(geometry, glyphMaterialMode, color);
}
...
export function renderMatcapPreviewDataUrl(mode, size = 64) {
  ...
  if (normalized === "depth") {
    ...
    previewCam.matrixWorldInverse.copy(previewCam.matrixWorld).invert();
    syncDepthMaterialsForCamera(previewCam, previewScene, rendererResolution(matcapPreviewRenderer));
  } else {
  ...
  if (normalized === "depth") {
    renderSceneWithOutline(matcapPreviewRenderer, previewScene, previewCam);
  } else {
...
/** Дефолтный глиф для превью маткапа — крупные штрихи хорошо показывают материал. */
const MATCAP_PREVIEW_GLYPH = "水";

/**
 * Превью маткапа на настоящем экструдированном глифе (те же материалы, что и текст),
 * а не на абстрактной сфере. Асинхронно, т.к. требует загрузки шрифта и геометрии.
 */
export async function renderGlyphMatcapPreviewDataUrl(mode, glyph = MATCAP_PREVIEW_GLYPH, size = 72) {
  const normalized = mode === "gold" ? "light" : mode;

  await ensureGlyphFontsReady();
  let geometry;
  try {
    geometry = await buildGlyphGeometry(glyph);
  } catch {
    return renderMatcapPreviewDataUrl(mode, size);
  }

  if (normalized === "light" || normalized === "badge") {
    ensureMatcapTexture(normalized);
  }
  ...
  const previewScene = new THREE.Scene();
  const previewCam = new THREE.PerspectiveCamera(38, 1, 1, 4000);
  previewCam.position.z = (size * 0.5) / Math.tan(19 * DEG);
  previewCam.lookAt(0, 0, 0);

  const previewRoot = createGlyphMeshForMode(geometry, normalized, 0xffffff);
  previewRoot.scale.setScalar((size * 0.82) / GLYPH_REF_PX);
  previewRoot.rotation.set(-0.3, 0.52, 0);
  previewScene.add(previewRoot);
...
async function buildGlyphGeometry(glyph) {
  ...
  const promise = (async () => {
    ...
    // Auto-smooth для основного меша: сглаживаем пологие рёбра (кривые контура,
    // фаска), но сохраняем острые углы (грани лицо↔бок, стыки штрихов) гранёными.
    const creased = toCreasedNormals(geometry, GLYPH_CREASE_ANGLE);
    geometry.dispose();

    geometryCache.set(key, creased);
    return creased;
  })();
...
export function renderGlyphThreeStage() {
  ...
  resizeStage();
  syncDepthMaterialsForCamera(camera, scene, rendererResolution(renderer));
  if (glyphMaterialMode === "depth") {
    renderSceneWithOutline(renderer, scene, camera);
  } else {
...
export function disposeGlyphThreeStage() {
  ...
  geometryCache.clear();
  if (renderer) outlineRTs.get(renderer)?.dispose();
